...
echo  [3] Exit
echo.
call :ReadSvcState "%CLIP_SVC%"     CLIP_SVC_STATE
call :ReadSvcState "%WPN_SYS_SVC%"  WPN_SYS_STATE
call :ReadSvcState "%WPN_USR_SVC%"  WPN_USR_STATE
call :ReadRegDword "%CLIP_POL_KEY%" "%CLIP_POL_VAL%" CLIP_POL_STATE
call :ReadRegDword "%TOAST_KEY%"    "%TOAST_VAL%"    TOAST_STATE

echo  --- Clipboard History ---
echo    cbdhsvc ...................... !CLIP_SVC_STATE!
echo    AllowClipboardHistory ........ !CLIP_POL_STATE!
echo  --- Notifications ---
...
:ENABLE
cls
echo Enabling Clipboard History + Notifications...
echo.
if exist "%FAIL_FLAG%" del "%FAIL_FLAG%" >nul 2>&1

call :ConfigService "%CLIP_SVC%"    "auto"
call :ConfigService "%WPN_SYS_SVC%" "auto"
call :ConfigService "%WPN_USR_SVC%" "auto"
call :SetRegDword "%CLIP_POL_KEY%" "%CLIP_POL_VAL%" 1
...
echo.
echo  Other apps (Mail, Store, Edge) also use the notification
echo  services. Disable them only if you don't need them at all.
echo.
choice /c yn /n /m "  Also disable Notifications? (Y/N): "
set "disable_notif=%errorlevel%"
:: 1=Y 2=N

echo.
if exist "%FAIL_FLAG%" del "%FAIL_FLAG%" >nul 2>&1

call :ConfigService "%CLIP_SVC%" "disabled"
call :SetRegDword "%CLIP_POL_KEY%" "%CLIP_POL_VAL%" 0
